...
*   **1️⃣ 작업 폴더 생성**
    ...
    *   `github.com` 폴더 하위에 **`vibecoding-naver-news`** 폴더를 새로 만듭니다.
*   **2️⃣ 안티그래비티 IDE 실행**
    *   메뉴에서 **파일 > 뉴 윈도우(New Window)**를 선택합니다.
    *   새로 만든 **`vibecoding-naver-news`** 폴더를 활성화하여 엽니다.
...
*   **1️⃣ 작업 폴더 생성**
    *   탐색기(Finder/Explorer)에서 작업할 폴더인 `github.com`을 생성합니다. (이미 존재한다면 생략 가능)
    *   `github.com` 폴더 하위에 **`vibecoding-petcare-app`** 폴더를 새로 만듭니다.
*   **2️⃣ 안티그래비티 IDE 실행**
    *   메뉴에서 **파일 > 뉴 윈도우(New Window)**를 선택합니다.
    *   새로 만든 **`vibecoding-petcare-app`** 폴더를 활성화하여 엽니다.
*   **3️⃣ 저장소 클론 (Git Clone)**
    *   채팅창에 다음과 같이 입력하여 템플릿 저장소를 복제합니다.
        ```text
        https://github.com/raymondbae/Practical-Vibe-Coding-Course.git 복사 해줘
